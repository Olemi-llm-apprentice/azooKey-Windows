...
### A-2) 一時ファイルからコミットメッセージを読み込む（PowerShell/改行問題回避）
...
cat > tools/git/commit_msg.txt <<'EOF'
fix: 不要なデバッグログ出力を削除
...
```bash
# 一時ファイルにタイトルと本文を書き出す
cat > tools/git/pr_title.txt <<'EOF'
fix: 不要なデバッグログ出力を削除
EOF

cat > tools/git/pr_body.txt <<'EOF'
## 概要
このPRでは、不要なデバッグログを削除し、ログ出力量を抑制します。
...
git commit -m "$MSG" && \
git push -u origin "$BRANCH" && \
gh pr create --title "$(cat tools/git/pr_title.txt)" --body-file tools/git/pr_body.txt --base main

# 一時ファイルを削除
rm tools/git/pr_title.txt tools/git/pr_body.txt
```
...
@"
fix: 不要なデバッグログ出力を削除
"@ | Out-File -FilePath tools/git/pr_title.txt -Encoding UTF8 -NoNewline
...
## 関連Issue
Refs #123
"@ | Out-File -FilePath tools/git/pr_body.txt -Encoding UTF8
...
git commit -m $MSG
git push -u origin $BRANCH
gh pr create --title (Get-Content tools/git/pr_title.txt -Raw).Trim() --body-file tools/git/pr_body.txt --base main

# 一時ファイルを削除
Remove-Item tools/git/pr_title.txt, tools/git/pr_body.txt
```

> **注意:** `gh pr create`には`--title-file`オプションはありませんが、ファイルから読み込んで`--title`に渡すことができます。`Get-Content`の`-Raw`オプションでファイル全体を読み込み、`.Trim()`で末尾の改行を削除します。
...
# 5) コミット（一時ファイルから読み込む方法も推奨）
# 方法1: 直接指定
MSG="fix: 不要なデバッグログ出力を削除"
git commit -m "$MSG"
# 方法2: 一時ファイルから読み込む（PowerShell推奨）
# cat > tools/git/commit_msg.txt <<'EOF'
# fix: 不要なデバッグログ出力を削除
# EOF
# git commit -F tools/git/commit_msg.txt
...
> **PowerShell推奨:** PowerShellから実行する場合は、文字化けリスクを回避するため、**A-2) 一時ファイルからコミットメッセージを読み込む方法**または**B-2) タイトルと本文を一時ファイルから読み込む方法**を使用してください。
